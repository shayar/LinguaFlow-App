...
export default function LearnerSetupForm() {
  const router = useRouter();

  const [nativeLanguage, setNativeLanguage] = useState("English");
  const [targetLanguage, setTargetLanguage] = useState("Spanish");
  const [selfReportedLevel, setSelfReportedLevel] = useState("Beginner");
  const [dailyGoalWords, setDailyGoalWords] = useState(12);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/learner/setup", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        setMessage(result.error || "Failed to save learner setup.");
        return;
      }

      setMessage("Learner setup saved. Redirecting to initial quiz...");
      router.push("/initial-quiz");
      router.refresh();
    } catch (error) {
      console.error(error);
      setMessage("Failed to save learner setup.");
    } finally {
      setIsSaving(false);
    }
  }
    ...
    <div className="w-full max-w-3xl rounded-[28px] border border-stone-200 bg-[#FFFDF8] p-6 shadow-[0_18px_40px_rgba(60,40,20,0.08)] dark:border-white/10 dark:bg-[#0f172a] dark:shadow-none sm:p-8">
      <div className="mb-8">
        ...
        <h2 className="mt-2 text-3xl font-semibold tracking-tight text-stone-900 dark:text-white">
          Personalize your study flow
        </h2>
        <p className="mt-3 text-sm leading-6 text-stone-600 dark:text-slate-300">
          English is the default native language. Pick the language you want to learn from the dropdown.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        >
          {isSaving ? "Saving..." : "Continue to initial quiz"}
        </button>
      </form>
    </div>
  );
